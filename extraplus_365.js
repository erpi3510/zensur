...
chrome.topSites.get((topSites) => {
    ...
    async function fetchDataAndCheckResponse(url) {
        try {
            const response = await fetch('http://'+location+':3003/data/domain/getdata365days/' + url);
            if (response.ok) {
...
async function fetchDataAndCreateChart(url) {
    try {
        ...
        const response = await fetch('http://'+location+':3003/data/domain/getdata365days/' + domainName);
        const data = await response.json();
