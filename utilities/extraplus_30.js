...
chrome.topSites.get((topSites) => {
    ...
    async function fetchDataAndCheckResponse(url) {
        try {
            const response = await fetch(location+'/data/domain/getdata30days/' + url);
            if (response.ok) {
...
async function updateDropdownMenu() {
    try {
        ...
        if (!valueUrl) {
            selectElement.innerHTML = '<option selected>'+chrome.i18n.getMessage("option5")+'</option>';
            return;
        }
...
async function fetchDataAndCreateChart(url) {
    try {
        ...
        const response = await fetch(location+'/data/domain/getdata30days/' + domainName);
        const data = await response.json();
        ...
        // Wähle jeden 7. Tag aus den Daten aus
        const filteredData = data.filter((_, index) => index % 7 === 0);
