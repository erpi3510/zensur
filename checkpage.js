...
function nextPage() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async function(tabs) {
        ...
        blockURLs([extractNameAndDomain(urls)]);
        chrome.tabs.update(tabId, { url: urls });
    
// Funktion zum Abrufen und Entfernen der blockierten URL anhand der Registerkarten-ID
...
chrome.tabs.query({ active: true, currentWindow: true }, async function(tabs) {
    ...
    var tabId = tabs[0].id;
    var urls = await getBlockedURL(tabId);
    document.getElementById("topurls").innerHTML = extractNameAndDomain(urls)+'da';
