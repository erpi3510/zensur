chrome.topSites.get((topSites) => {
    ...
    const select = document.getElementById('dynamicSelect365');
    ...
    async function fetchDataAndCheckResponse(url) {
        try {
            const response = await fetch('http://localhost:3003/data/domain/getdata365days/' + url);
            if (response.ok) {
...
document.addEventListener('DOMContentLoaded', function () {
    const selectElement = document.getElementById('dynamicSelect365');
...
async function fetchDataAndCreateChart(url) {
    try {
        ...
        const response = await fetch('http://localhost:3003/data/domain/getdata365days/' + domainName);
        const data = await response.json();
        ...
        data.forEach(entry => {
            chartData.labels.push(translateMonth(entry.month)+' '+entry.year);
        ...
        // Chart erstellen
        const ctx = document.getElementById('myChart365').getContext('2d');
        new Chart(ctx, {
            type: 'line',
            data: chartData,
            options: options
...
function translateMonth(month) {
    ...
    return months[month - 1]; // Da die Monate in JavaScript bei 0 beginnen
}